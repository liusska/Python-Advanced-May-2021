...
def buy_product(button):
    ...
    current_logger_in_user = None
    with open("db/current_user.txt", "r") as file:
        current_logger_in_user = file.read()
    ...
    with open("db/users.txt", "r+") as file:
        ...
        for user in users:
            ...
            if user_as_dict.get("username") == current_logger_in_user:
...
def render_products():
    ...
    with open("db/products.txt", "r") as file:
        ...
        for product in products:
            ...
            Label(text=current_product.get('name'))\
                .grid(row=0, column=column_counter)
